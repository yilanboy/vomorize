<script lang="ts">
    ...
    import { currentLocaleUrlKey, interpolate, localized, translations } from '@/lib/locale.svelte';
    ...
    let breadcrumbs = $derived<Breadcrumb[]>([
        { label: t['home'], url: homeRoute.url({ locale: currentLocaleUrlKey() }) },
        {
            label: localized<LevelTranslationItem>(level.translations)?.name ?? '',
            url: levelRoute.url({ locale: currentLocaleUrlKey(), level: level.id }),
        },
