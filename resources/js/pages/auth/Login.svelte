...
<Form {...store.form()} resetOnSuccess={['password']} class="flex flex-col gap-6">
    {#snippet children({ errors, processing })}
        <div class="grid gap-4">
            ...
            <div class="grid gap-2.5">
                <div class="flex items-center justify-between">
                    ...
                    {#if canResetPassword}
                        <TextLink href={request({ locale: currentLocaleUrlKey() })} class="text-sm"
            ...
            <Button variant="outline" class="w-full" asChild>
                {#snippet children(props)}
                    <a href="/auth/github/redirect" {...props}>
        ...
        <div class="text-center text-sm text-zinc-500 dark:text-zinc-400">
            {t['no_account']}
            <TextLink href={register({ locale: currentLocaleUrlKey() })}>{t['sign_up']}</TextLink>
        </div>
    {/snippet}
</Form>
